...
public class SceneChunk : MonoBehaviour
{
    ...
    protected void PrepareSceneBlocks()
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            int worldIndex = Helpers.IndexFromVec2Int(worldPos, ClientGameManager.Singleton.Mapdata._mapSize.x);
    ...
    public void UpdateBlockLightWithNeighborsWalls(MapData.MapTileData blockData, int localTileIndex)
    {
        ...
        {
            SceneLevelManager.Singleton.UpdateNeighborChunkWallLight(block.WorldPosIndex + ClientGameManager.Singleton.Mapdata._mapSize.x, new Vec2Int(block.WorldPos.x, block.WorldPos.y + 1), GameData.GameBlockData.BlockFaces.FaceZBack, blockData.LightColor);
        }
        else
        {
        ...
        {
            SceneLevelManager.Singleton.UpdateNeighborChunkWallLight(block.WorldPosIndex - ClientGameManager.Singleton.Mapdata._mapSize.x, new Vec2Int(block.WorldPos.x, block.WorldPos.y - 1), GameData.GameBlockData.BlockFaces.FaceZForward, blockData.LightColor);
        }
        else
        {
    ...
    private SceneChunkRenderer GetSceneChunkRenderer(uint scrUID)
    {
        ...
        {
            Material mat = null;
            ClientGameManager.Singleton.Gamedata.GetMaterial(scrUID, out mat);
    ...
    private SceneChunkScaffoldRenderer GetSceneChunkScaffoldRenderer(uint sscrUID)
    {
        ...
        {
            Material mat = null;
            ClientGameManager.Singleton.Gamedata.GetMaterial(sscrUID, out mat);
